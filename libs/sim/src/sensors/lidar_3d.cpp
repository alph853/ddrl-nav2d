...
//=============================================================================
// Lidar3DSensor Implementation
//=============================================================================

class Lidar3DSensor::DerivedImpl
{
public:
  ...
  [[nodiscard]] const std::shared_ptr<const core::sensors::Lidar3D>& config_lidar() const
  {
...
private:
  std::shared_ptr<const core::sensors::Lidar3D> config_lidar_;
  std::mt19937                                  rng_;
  std::vector<collision::RayRequest>            ray_buffer_;
  std::vector<collision::RayHit>                hit_buffer_;
};

Lidar3DSensor::DerivedImpl::DerivedImpl(const SensorBase::Params& params)
{
  if (!std::holds_alternative<core::sensors::Lidar3D>(*params.config)) {
    throw std::runtime_error("Expected Lidar3D config in Lidar3DSensor constructor");
  }

  const auto& lidar3d_variant = std::get<core::sensors::Lidar3D>(*params.config);
  config_lidar_               = std::make_shared<const core::sensors::Lidar3D>(lidar3d_variant);

  std::random_device rd;
  rng_.seed(rd());
}

void Lidar3DSensor::DerivedImpl::process_hits(
  std::span<const collision::RayHit> hits, const core::math::Pose3& sensor_pose_world,
  core::vision::PointCloud& out_cloud, SensorBase& base
)
{
...
Lidar3DSensor::Lidar3DSensor(const Params& params)
    : SensorBase(params), derived_impl_(std::make_unique<DerivedImpl>(params))
{
}

Lidar3DSensor::~Lidar3DSensor() = default;

Lidar3DSensor::Lidar3DSensor(Lidar3DSensor&&) noexcept            = default;
Lidar3DSensor& Lidar3DSensor::operator=(Lidar3DSensor&&) noexcept = default;

std::shared_ptr<const core::vision::PointCloud> Lidar3DSensor::get_point_cloud() const
{
  return get_cached_cloud();
}

Result<void> Lidar3DSensor::update(double dt, double current_time)
{
  ...
  collision::generate_lidar_3d_rays(sensor_pose_world, *derived_impl_->config_lidar(), rays);
